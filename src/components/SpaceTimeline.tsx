...
export default function SpaceTimeline() {
  ...
  return (
    <section className="py-10 md:py-20 bg-gradient-to-br from-purple-100 to-blue-100 relative overflow-hidden">
              ...
              <svg width="8" height="100%" className="absolute" preserveAspectRatio="none">
                <defs>
                  <pattern id="wavyLine" x="0" y="0" width="8" height="200" patternUnits="userSpaceOnUse">
                    <path d="M4,0 Q8,50 4,100 Q0,150 4,200" stroke="#000" strokeWidth="4" fill="none" />
                  </pattern>
                </defs>
          ...
          <div className="space-y-8 md:space-y-20">
            {timelineEvents.map((event, index) => (
              ...
              >
                {/* Timeline dot */}
                <div
                  className={`absolute left-0 md:left-1/2 md:transform md:-translate-x-1/2 w-10 h-10 md:w-16 md:h-16 ${event.color} border-2 md:border-4 border-black shadow-xl z-20 flex items-center justify-center rounded-full md:rounded-none hover:scale-125 transition-transform`}
                >
                  <div className="text-black">{getIcon(event.type)}</div>
                </div>

                {/* Mobile layout */}
                <div className="md:hidden ml-16 space-y-4">
                  <div className="bg-white border-4 border-black p-4 shadow-xl hover-lift transform rotate-1 hover:rotate-0">
                    <div className="flex items-center gap-3 mb-3">
                      <div
                        className={`px-4 py-2 ${event.color} border-2 border-black font-black text-lg transform -rotate-2`}
                      >
                        {event.year}
                      </div>
                    </div>
                    ...
                    <p className="text-gray-700 leading-relaxed font-semibold text-sm">{event.description}</p>
                  </div>

                  <div className="aspect-video overflow-hidden border-4 border-black shadow-xl hover-lift transform -rotate-1 hover:rotate-0">
                    <img
                ...
                >
                  <div
                    className={`bg-white border-4 border-black p-6 lg:p-8 shadow-xl hover-lift transform ${index % 2 === 0 ? "rotate-2" : "-rotate-2"} hover:rotate-0`}
                  >
                    <div
                      className={`flex items-center gap-3 mb-4 ${index % 2 === 0 ? "justify-end" : "justify-start"}`}
                    >
                      <div
                        className={`px-4 lg:px-6 py-2 lg:py-3 ${event.color} border-3 border-black font-black text-lg lg:text-xl transform -rotate-3`}
                      >
                        {event.year}
                      </div>
                    </div>
                    ...
                    <p className="text-gray-600 text-sm mb-4 flex items-center gap-2 font-bold">
                      <MapPin className="w-4 h-4" />
                      {event.location}
                    </p>
                    <p className="text-gray-700 leading-relaxed font-semibold">{event.description}</p>
                  </div>
                </div>
                ...
                <div className={`hidden md:block md:w-5/12 ${index % 2 === 0 ? "md:pl-16" : "md:pr-16"}`}>
                  <div
                    className={`aspect-video overflow-hidden border-4 border-black shadow-xl hover-lift transform ${index % 2 === 0 ? "-rotate-2" : "rotate-2"} hover:rotate-0`}
                  >
                    <img
                      src={event.image || "/placeholder.svg"}
                      alt={event.title}
